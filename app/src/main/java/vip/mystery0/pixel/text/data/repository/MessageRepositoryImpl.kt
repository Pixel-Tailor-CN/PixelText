...
class MessageRepositoryImpl(
            ...
            while (cursor.moveToNext()) {
                ...
                if (!messagesMap.containsKey(threadId)) {
                    ...
                    messagesMap[threadId] = ConversationModel(
                        threadId = threadId,
                        address = address,
                        displayName = getContactDisplayName(address),
                        snippet = body,
                        timestamp = date,
    ...
    private fun ensureContactNameCacheLoaded() {
        ...
        try {
            ...
            Log.w("MessageRepository", "missing contacts permission while loading contact names")
        } catch (e: Exception) {
            Log.e("MessageRepository", "failed to load contact names", e)
        }
    }
