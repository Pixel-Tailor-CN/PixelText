...
- [ ] Observer 回调只标 dirty/调度；收到/发送/已读/删除/下载完成写系统成功后追加定向 dirty，Receiver 使用现有安全生命周期且不执行全库复制。
...
- [ ] 回调或恢复先检查源记录是否已被删除；删除后不复活、不再写文件。任务结束清临时授权和 PDU 临时文件。
...
- [ ] 保留发送、删除、已读系统写操作；成功后定向同步并更新 Flow，不对镜像做猜测式最终写入。
