...
普通对账中的 PARTIAL、预算让出和待处理变更不再直接映射为失败警告。横幅等待真实数据库状态，只在首次基础导入尚未完成，或存在明确集合错误、缺失子结构、附件复制失败时显示；候选提示持续 1 秒才展示，恢复正常时立即隐藏。已完成首次导入后的后台对账和手动刷新保持静默，沿用页面已有下拉刷新指示器。

验证：`:app:assembleDebug` 与 `:app:lintDebug` 成功。更新模拟器 APK 后，重新进入应用、下拉刷新均未出现同步/失败横幅，日志确认 SMS/MMS 对账完成且 dirty_count=0。未通过清空现有数据库模拟首次导入，也未注入真实 Provider 失败；相关提示分支完成静态核查。
