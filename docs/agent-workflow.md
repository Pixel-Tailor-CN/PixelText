...
重复出现且有项目专属步骤的操作放入 `.agents/skills/<name>/SKILL.md`，通过简短、准确的 `description` 匹配任务。
当前 `pixeltext-validation` 技能用于 Android 改动验证，不要求文档任务加载。
...
2026-09-12 检查时，仓库没有自有 SKILL.md，通用技能来自用户个人目录及插件。本次新增项目验证技能，不复制或改写个人技能。
